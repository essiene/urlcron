...
new_named_schedule_test() ->
    ...
    Url = "url",
    {ok, {Name, Starttime, Url}} = urlcron_scheduler:create(Name, Starttime, Url).

new_anonymous_schedule_test() ->
    Starttime = urlcron_util:get_future_time(600000),
    Url = "url",
    {ok, {"schedule." ++ _Details, Starttime, Url}} = urlcron_scheduler:create(Starttime, Url).

get_status_test() ->
    StartTime = urlcron_util:get_future_time(60000),
    Url = "http://boo.com",
    {ok, {"schedule02", StartTime, Url}} = urlcron_scheduler:create("schedule02", StartTime, Url),

    Result = urlcron_scheduler:get("schedule02"),
